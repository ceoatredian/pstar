<?php $this->load->view('common/header'); ?>
	 <script>
			...
			$("#vdrat .vstar").click(function () {
			var label = $("label[for='" + $(this).attr('id') + "']");
			 $.ajax({
						url: "<?php echo base_url();?>" + 'user/video_rating/<?php echo $user_profile->ID; ?>',
						type:'POST',										
						data: "rating="+$(this).val(),
						success:function(response){
							if(response.exists == "1") 
    ...
    <div class="container">
		<div class="row user-profile">
			...
			<?php } ?>
				...
				<p><?php // print_r($album_id);die; echo $user_profile->introduction;?></p>
						...
						<div class="row">
						 ...
						 <?php foreach ($album_detail as $row): ?>
							<div class="col-sm-4"">
                               ...
                               <source src="<?php echo HTTP_IMAGES_PATH.$row['video_path']; ?>">
                               </video>
																	 <p align="center"> Rate This Video</p> 
									<?php $ratt=$row['rating'];  $rating=round($ratt); ?>
                                    <fieldset id='vdrat' class="rating" style="margin-left:70px;">
                                        <input class="vstar" type="radio" id="star53.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='5'){ ?> checked="checked" <?php } ?> value="5,<?php echo $row['id']; ?>" />
                                        <label class = "full" for="star53.5<?php echo $row['id']; ?>" title="Awesome - 5 stars"></label>
                        
                                        <input class="vstar" type="radio" id="star4half3.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='4.5'){ ?> checked="checked" <?php } ?> value="4.5,<?php echo $row['id']; ?>" />
                                        <label class="half" for="star4half3.5<?php echo $row['id']; ?>" title="Pretty good - 4.5 stars"></label>
										
										<input class="vstar" type="radio" id="star43.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='4'){ ?> checked="checked" <?php } ?> value="4,<?php echo $row['id']; ?>" />
                                        <label class = "full" for="star43.5<?php echo $row['id']; ?>" title="Pretty good - 4 stars"></label>
						                 
						                <input class="vstar" type="radio" id="star3half3.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='3.5'){ ?> checked="checked" <?php } ?> value="3.5,<?php echo $row['id']; ?>" />
                                        <label class="half" for="star3half3.5<?php echo $row['id']; ?>" title="Meh - 3.5 stars"></label>
						
                                        <input class="vstar" type="radio" id="star33.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='3'){ ?> checked="checked" <?php } ?> value="3,<?php echo $row['id']; ?>" />
                                        <label class = "full" for="star33.5<?php echo $row['id']; ?>" title="Meh - 3 stars"></label>
                        
						                <input class="vstar" type="radio" id="star2half3.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='2.5'){ ?> checked="checked" <?php } ?> value="2.5,<?php echo $row['id']; ?>" />
                                        <label class="half" for="star2half3.5<?php echo $row['id']; ?>" title="Kinda bad - 2.5 stars"></label>
                        
						                <input class="vstar" type="radio" id="star23.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='2'){ ?> checked="checked" <?php } ?> value="2,<?php echo $row['id']; ?>" />
                                        <label class = "full" for="star23.5<?php echo $row['id']; ?>" title="Kinda bad - 2 stars"></label>
                        
						                <input class="vstar" type="radio" id="star1half3.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='1.5'){ ?> checked="checked" <?php } ?> value="1.5,<?php echo $row['id']; ?>" />
                                        <label class="half" for="star1half3.5<?php echo $row['id']; ?>" title="Meh - 1.5 stars"></label>
                        
						                <input class="vstar" type="radio" id="star13.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='1'){ ?> checked="checked" <?php } ?> value="1,<?php echo $row['id']; ?>" />
                                        <label class = "full" for="star13.5<?php echo $row['id']; ?>" title="Sucks big time - 1 star"></label>
                        
						                <input class="vstar" type="radio" id="starhalf3.5<?php echo $row['id']; ?>" name="rating<?php echo $row['id']; ?>" <?php if($rating=='.5'){ ?> checked="checked" <?php } ?> value="0.5,<?php echo $row['id']; ?>" />
